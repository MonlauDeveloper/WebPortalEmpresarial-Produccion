<div x-data>
                ...
                <div class="flex flex-col mt-7 border divide-y rounded-md">
                    @foreach ($this->categories as $category) 
                        <div class="flex items-center gap-5 p-5">
                            ...
                            <span wire:click="deleteCategory('{{ $category->id }}')" class="material-symbols-outlined hover:text-blue-500 transition-all cursor-pointer">delete</span>
                        </div>
                    @endforeach
                </div>
            @else
                    ...
                    @foreach ($this->products as $product)
                        <tr class="bg-white border-b">
                            ...
                            <td class="px-6 py-4 gap-5 flex items-center justify-end">
                                ...
                                <span wire:click="delete('{{ $product['id'] }}')" class="material-symbols-outlined hover:text-blue-500 transition-all cursor-pointer">delete</span>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <x-modal wire:model="deleting" styles="flex flex-col gap-5">
        <h2>¿Estás seguro de que quieres borrar este producto?</h2>
    ...
    {{-- @ Delete Category --}}
    <x-modal wire:model="deleting_category" styles="flex flex-col gap-5">
        <h2>¿Estás seguro de que quieres borrar esta categoría?</h2>

        <div class="flex items-center gap-5 w-full"> 
            <x-button wireClick="deleteCategory('{{ $deleting_category }}')" styles="w-[50%]" content="Si, estoy seguro" />
            <x-button wireClick="$set('deleting_category', false)" content="No, cancelar" styles="bg-black hover:bg-black w-[50%]" />
        </div>
    </x-modal>
